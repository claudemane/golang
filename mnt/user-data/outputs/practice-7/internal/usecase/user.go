...
// GetMe returns the full user record for the authenticated caller.
// The userID is extracted from the JWT by the controller layer.
...
// PromoteUser sets the target user's role to "admin".
// Only an existing admin may call this endpoint (enforced by RoleMiddleware).
